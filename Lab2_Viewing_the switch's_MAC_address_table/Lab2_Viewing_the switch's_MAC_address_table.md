...
**Ответ:** Можно определить принадлежность по порту подключения - устройство, подключенное к порту Fa0/1, это коммутатор S1. Однако это решение работает не в любой ситуации, особенно когда к одному порту подключено несколько устройств через другие коммутаторы.
...
**Ответ:** Нет, новые адреса не появились. MAC-адрес S1 остался в таблице из-за постоянного обмена служебными кадрами между коммутаторами (протокол STP).
